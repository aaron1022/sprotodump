...
local function dump_class(class_info, stream, deep, namespace)
  ...
  if sproto_type then
    ...
    for i=1,#internal_class do
      dump_class(internal_class[i], stream, deep, namespace)
    end
          ...
          if func_name then
            if field.array then
              stream:write(sformat("if(_%s != null){", name), deep + 1)
                stream:write(sformat("tTable.AddTable(\"%s\");" , name), deep + 1)
                stream:write(sformat("var tT = (LuaInterface.LuaTable)tTable[\"%s\"];", name), deep + 1)
                stream:write(sformat("for(int i = 0; i < _%s.Count; i++)", name), deep + 2)
                  if string.match(func_name, "read_integer") == true then
              ...
              stream:write(sformat("if(base.has_field.has_field(%s) == true)", i - 1), deep + 1)
    ...
    stream:write("", deep)
    for i=1,#internal_class do
      dump_class(internal_class[i], stream, deep+1, namespace)
    end
    stream:write("}\n\n", deep)
  end
end
...
local function parse_type(class, stream, package)
  ...
  for i=1,#class do
    ...
    dump_class(class_info, stream, 1, namespace)
  end
